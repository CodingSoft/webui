...
class AppConfig:
    ...
    def __init__(
        ...
        redis_key_prefix: str = "codingsoft-webui",
    ):
        if redis_url:
        ...
        if r.ok:
            if "logo" in data:
                WEBUI_FAVICON_URL = url = (
                    f"https://api.webui.codingsoft.org{data['logo']}"
                    if data["logo"][0] == "/"
                    else data["logo"]
                )
            ...
            if "splash" in data:
                url = (
                    f"https://api.webui.codingsoft.org{data['splash']}"
                    if data["splash"][0] == "/"
                    else data["splash"]
                )
...
if ENV == "prod":
    if OLLAMA_BASE_URL == "/ollama" and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == "true":
            # if you use all-in-one docker container (CodingSoft WebUI + Ollama)
...
PINECONE_INDEX_NAME = os.getenv("PINECONE_INDEX_NAME", "codingsoft-webui-index")
