<script lang="ts">
	...
	const DEFAULT_PINNED_ITEMS = ['notes', 'workspace', 'projects', 'files'];
	...
	const getMenuItemMeta = (id) => {
		const items = {
			...
			projects: { label: 'Projects', href: '/projects', iconType: 'folder' },
			files: { label: 'Files', href: '/files', iconType: 'document' }
		};
		return items[id];
	};
